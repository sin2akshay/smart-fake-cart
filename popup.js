...
const TOOL_DEFS = [
  {
    ...
    description: "Extracts product name, current price, rating, and store from the active Amazon/Flipkart tab. Call this FIRST.",
    params: {
      type: "object",
  ...
  },
  {
    ...
    description: "Returns 90-day price history stats: average, min, max, 7-day trend, verdict, and recommended buy price.",
    params: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Sets a Chrome alarm to monitor price every 60 min and send a notification when it drops to or below the threshold. Call this when price is above recommended buy price.",
    params: {
      type: "object",
      properties: {
      ...
      required: ["product_url", "product_name", "threshold_price", "current_price"]
    }
  },
  {
    name: "check_discount_coupons",
    description: "Checks available discount coupons, bank card offers, cashback deals, and EMI options for a product on Amazon or Flipkart. ALWAYS call this as step 3, after check_price_history. Returns a list of applicable offers and the best effective price after discounts.",
    params: {
      type: "object",
      properties: {
        product_name:  { type: "string", description: "Product name to find coupons for" },
        current_price: { type: "number", description: "Current listed price" },
        store:         { type: "string", description: "Store name: Amazon or Flipkart" },
...
function sbAddTokens(input = 0, output = 0) {
  ...
  sbTokens.textContent = totalTokens >= 1000
...
document.querySelectorAll(".tab").forEach(tab => {
  tab.addEventListener("click", () => {
    ...
    if (tab.dataset.tab === "logs") logTerminal.scrollTop = logTerminal.scrollHeight;
  });
});

btnCopyLogs?.addEventListener("click", () => {
  const text = logLines.map(l => `[${l.ts}] [${l.level.toUpperCase().padEnd(4)}] ${l.msg}`).join("\n");
...
function logDone() {
  ...
  chrome.storage.local.set({
    lastSession: { chainHtml: chainEl.innerHTML, logLines, url: productUrlInput.value }
  });
}
...
(async () => {
  const { apiKey, geminiModel, lastSession } = await chrome.storage.local.get(
    ["apiKey","geminiModel","lastSession"]
  );
  if (geminiModel) [...modelSelect.options].forEach(o => { if (o.value === geminiModel) o.selected = true; });
  ...
  if (lastSession?.chainHtml) {
    ...
    chainEl.querySelectorAll(".step-card").forEach(card => {
      ...
      if (tag) tag.addEventListener("click", () => card.classList.toggle("collapsed"));
    });
...
// ══════════════════════════════════════════════════════════════════
function showSetup() { screenSetup.classList.remove("hidden"); screenMain.classList.add("hidden"); }
function showMain() {
  screenSetup.classList.add("hidden"); screenMain.classList.remove("hidden");
  chrome.storage.local.get("geminiModel").then(({ geminiModel }) => {
    const m = (geminiModel || "gemini-3.1-flash-lite-preview").replace("gemini-","");
...
btnFetchModels?.addEventListener("click", async () => {
  ...
  if (!key) { fetchStatus.textContent = "Enter your API key first."; return; }
  btnFetchModels.textContent = "Fetching…"; btnFetchModels.disabled = true; fetchStatus.textContent = "";
  try {
    ...
    if (!models.length) { fetchStatus.textContent = "No models found."; return; }
  ...
  } finally { btnFetchModels.textContent = "↻ Fetch from API"; btnFetchModels.disabled = false; }
});

btnSaveKey.addEventListener("click", async () => {
  const key = apiKeyInput.value.trim();
  if (!key.startsWith("AI")) { flash("Invalid key — should start with AIza…"); return; }
  await chrome.storage.local.set({ apiKey: key, geminiModel: modelSelect.value });
  showMain();
});

btnSettings.addEventListener("click", async () => {
  const { apiKey, geminiModel } = await chrome.storage.local.get(["apiKey","geminiModel"]);
...
btnAnalyze.addEventListener("click", async () => {
  ...
  if (!url.includes("amazon") && !url.includes("flipkart")) { flash("Amazon / Flipkart only!"); return; }
  ...
  logCountEl.textContent = "0";
  logDot.classList.remove("active");
  log("info", `▶ Starting agent — model: ${geminiModel || "gemini-3.1-flash-lite-preview"}`);
  sbStartRun();

  try {
    await runGemini(url, apiKey, geminiModel || "gemini-3.1-flash-lite-preview");
  } catch (err) {
...
async function runGemini(productUrl, apiKey, model) {
  const sysInstr = { parts: [{ text: "You are a smart shopping agent. You MUST call tools in this exact order: 1) scrape_product_page, 2) check_price_history, 3) check_discount_coupons, 4) set_price_alert ONLY if price is above recommended buy price. Never skip steps 1-3. Use markdown: **bold**, ### sections, - bullets." }] };
  const contents = [{
    role: "user",
    parts: [{ text: `Product URL: ${productUrl}\n\nTask — follow ALL steps in order:\n1. scrape_product_page → get name & current price\n2. check_price_history → 90-day price analysis\n3. check_discount_coupons → find bank offers, coupons, cashback (ALWAYS do this step)\n4. If price is still above recommended buy price after discounts, call set_price_alert\n5. Final verdict: BUY NOW or WAIT, showing best effective price after coupons.` }]
  }];
  ...
  while (loops++ < 8) {
    ...
    log("data", `model=${model} | maxOutputTokens=1500 | tools=${GEMINI_TOOLS[0].functionDeclarations.length} | turns=${contents.length}`);
    ...
    const raw = await apiFetch(`${endpoint}?key=${apiKey}`, {
      ...
      body: JSON.stringify({
        contents, systemInstruction: sysInstr, tools: GEMINI_TOOLS,
    ...
    log("resp", `← Response in ${ms}ms | candidates=${raw.candidates?.length ?? 0} | finish="${cand?.finishReason ?? "?"}"`);
    if (raw.usageMetadata) {
      log("data", `tokens: prompt=${raw.usageMetadata.promptTokenCount} output=${raw.usageMetadata.candidatesTokenCount}`);
      sbAddTokens(raw.usageMetadata.promptTokenCount || 0, raw.usageMetadata.candidatesTokenCount || 0);
    }

    if (!cand) { addError("No response from Gemini: " + JSON.stringify(raw)); break; }
    ...
    if (!fnCalls.length) {
      log("info", `✅ Agent finished (no more function calls) after ${loops} LLM call(s)`);
      break;
    }
    ...
    log("info", `📤 Sending ${responseParts.length} function response(s) back to LLM`);
...
async function executeTool(name, input, ctxUrl) {
  switch (name) {
    case "scrape_product_page":     return toolScrape(input.url || ctxUrl);
    case "check_price_history":     return toolPriceHistory(input.product_name, input.current_price, input.currency || "INR");
    case "check_discount_coupons":  return toolCheckCoupons(input.product_name, input.current_price, input.store || "Amazon", input.currency || "INR");
    case "set_price_alert":         return toolSetAlert(input.product_url || ctxUrl, input.product_name, input.threshold_price, input.current_price, input.currency || "INR");
...
async function toolScrape(url) {
  ...
  } catch { [tab] = await chrome.tabs.query({ active: true, currentWindow: true }); }
  if (!tab) return { success: false, error: "No product tab found. Open the product page first." };
  ...
  try {
    const [{ result }] = await chrome.scripting.executeScript({
      ...
      func: () => {
        ...
        if (isAZ) {
          const t = document.querySelector("#productTitle") || document.querySelector(".product-title-word-break");
          const p = document.querySelector(".a-price-whole") || document.querySelector(".priceToPay .a-price-whole") || document.querySelector(".apexPriceToPay .a-price-whole");
          title = t?.textContent.trim() || "";
          if (p) price = parseInt(p.textContent.replace(/\D/g,""), 10) || null;
          return { success: !!(title && price), store: "Amazon", title, price, currency: location.hostname.includes(".in") ? "INR" : "USD", url: location.href };
        }
        const t = document.querySelector(".B_NuCI") || document.querySelector("h1.yhB1nd");
        const p = document.querySelector("._30jeq3._16Jk6d") || document.querySelector("._30jeq3") || document.querySelector(".Nx9bqj");
        title = t?.textContent.trim() || "";
        if (p) price = parseInt(p.textContent.replace(/\D/g,""), 10) || null;
        return { success: !!(title && price), store: "Flipkart", title, price, currency: "INR", url: location.href };
      }
    });
...
async function toolPriceHistory(productName, currentPrice, currency = "INR") {
  log("tool", `📊 check_price_history: "${productName.slice(0,50)}" @ ${currency} ${currentPrice}`);
  let seed = [...productName].reduce((a, c) => a + c.charCodeAt(0), 0);
  const rand = () => { seed = (seed * 1664525 + 1013904223) & 0xffffffff; return (seed >>> 0) / 0xffffffff; };

  const days = 90, basePrice = Math.round(currentPrice * (1.05 + rand() * 0.30));
  const prices = Array.from({ length: days + 1 }, (_, i) => {
    const v = (rand() - .5) * .08 * basePrice;
    const s = rand() < .04 ? .70 + rand() * .15 : 1;
    const d = 1 - (days - i) * .0008;
    return Math.max(Math.round((basePrice + v) * s * d), Math.round(currentPrice * .75));
  });
  ...
  const trend = last7 < prev7 ? "📉 Falling" : last7 > prev7 ? "📈 Rising" : "➡ Stable";
  const vsAvg = (((currentPrice - avg) / avg) * 100).toFixed(1);
  const vsMin = (((currentPrice - min) / min) * 100).toFixed(1);

  let verdict, recommendation, recommendedBuyPrice = Math.round(avg * 0.88);
  if      (currentPrice <= min * 1.03) { verdict = "EXCELLENT"; recommendation = "BUY NOW. Best price in 90 days."; recommendedBuyPrice = currentPrice; }
  else if (currentPrice <= avg * 0.92) { verdict = "GOOD";      recommendation = "BUY NOW. Well below average price."; recommendedBuyPrice = currentPrice; }
  else if (currentPrice <= avg * 1.02) { verdict = "FAIR";      recommendation = "Could wait. Price may dip lower."; }
  else                                  { verdict = "EXPENSIVE"; recommendation = "WAIT. Price is inflated. Set an alert."; }

  log("tool", `   Verdict: ${verdict} | avg=${currency}${avg} min=${currency}${min} trend=${trend}`);
  return { productName, currentPrice, currency, avgPrice90d: avg, minPrice90d: min, maxPrice90d: max, vsAverage: `${vsAvg > 0 ? "+" : ""}${vsAvg}%`, vsAllTimeLow: `+${vsMin}%`, trend7d: trend, verdict, recommendation, recommendedBuyPrice };
}

async function toolCheckCoupons(productName, currentPrice, store = "Amazon", currency = "INR") {
  log("tool", `🏷️ check_discount_coupons: "${productName.slice(0,50)}" on ${store}`);

  // Seeded random so same product always gets same offers
  let seed = [...(productName + store)].reduce((a, c) => a + c.charCodeAt(0), 0);
  const rand = () => { seed = (seed * 1664525 + 1013904223) & 0xffffffff; return (seed >>> 0) / 0xffffffff; };
  ...
  const bankOffers = [
    { bank: "HDFC Bank",  type: "Credit Card", pct: 10, cap: 1500, code: "HDFC10"   },
    { bank: "SBI",        type: "Debit Card",  pct: 8,  cap: 1000, code: "SBI8"     },
    { bank: "ICICI Bank", type: "Credit Card", pct: 7,  cap: 1200, code: "ICICI7"   },
    { bank: "Axis Bank",  type: "Credit Card", pct: 5,  cap: 800,  code: null       },
    { bank: "Kotak",      type: "Debit Card",  pct: 6,  cap: 750,  code: "KOTAK6"   },
    { bank: "IDFC First", type: "Credit Card", pct: 12, cap: 2000, code: "IDFC12"   },
    { bank: "IndusInd",   type: "Credit Card", pct: 9,  cap: 1100, code: "INDUS9"   },
  ];
  ...
  for (const b of bankPool) {
    ...
    selectedBanks.push({ ...b, discount, effectivePrice: currentPrice - discount });
  }
  ...
  const cbAmount = Math.min(Math.round(currentPrice * cb.pct / 100), cb.maxCB);
  ...
  log("tool", `   Found ${selectedBanks.length} bank offers, ${selectedCoupons.length} coupons, 1 cashback | Best: ${sym}${bestEffectivePrice.toLocaleString()} (save ${savingsPct}%)`);
...
async function toolSetAlert(url, productName, thresholdPrice, currentPrice, currency = "INR") {
  log("tool", `🔔 set_price_alert: threshold=${currency}${thresholdPrice} current=${currency}${currentPrice}`);
  const alertId = `alert_${Date.now()}`;
  const alert = { id: alertId, productUrl: url, productName, thresholdPrice, currentPrice, currency, createdAt: Date.now(), lastChecked: null, lastPrice: currentPrice, triggered: false, checkHistory: [] };
  const { alerts = {} } = await chrome.storage.local.get("alerts");
  alerts[alertId] = alert;
  await chrome.storage.local.set({ alerts });
  await chrome.runtime.sendMessage({ action: "SET_ALARM", alertId, periodInMinutes: ALARM_PERIOD });
  log("tool", `   ✓ Chrome alarm created: id=${alertId} period=${ALARM_PERIOD}min`);
  loadAlerts();
  return { success: true, alertId, thresholdPrice, currency, message: `Alert set at ${currency} ${thresholdPrice.toLocaleString()}. Checking every ${ALARM_PERIOD} min.` };
}
...
function addUserStep(url) {
  ...
  const path   = (() => { try { const p = new URL(url).pathname; return p.length > 36 ? p.slice(0,36)+"…" : p; } catch { return ""; } })();
  appendCard(`
...
function addAIStep(modelName, rawText) {
  ...
  let bannerHtml = "";
  if (isBuy)  bannerHtml = `<div class="verdict-banner verdict-buy"><span class="verdict-icon">✅</span> Recommended: BUY NOW</div>`;
  if (isWait) bannerHtml = `<div class="verdict-banner verdict-wait"><span class="verdict-icon">⏳</span> Recommended: WAIT for better price</div>`;
...
function renderHistoryResult(r) {
  ...
  const verdictColor = r.verdict === "EXCELLENT" || r.verdict === "GOOD" ? "good"
                     : r.verdict === "EXPENSIVE" ? "bad" : "warn";
  return `
    <div class="tool-result-grid">
      <div class="tool-result-item">
...
function renderCouponsResult(r) {
  ...
  const bankRows = r.bankOffers.map(b => `
  ...
  const couponRows = r.coupons.map(c => `
    <div class="coupon-row">
...
async function loadAlerts() {
  ...
  if (!list.length) { alertsPanel.classList.add("hidden"); return; }
...
function flash(msg) {
  ...
  statusText.textContent = msg;
  statusText.className = "sb-status error";
  setTimeout(() => { statusText.textContent = orig; statusText.className = origClass; }, 2000);
}
